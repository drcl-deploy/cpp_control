#!/usr/bin/env python3
"""
Run the offboard RGB-D viewer and snapshot labeler for Sys1 color calibration.
...
Sys1CalibrationState carries the exact source sequence and ground-truth label.
...
from cpp_control.msg import Sys1CalibrationState, Sys1Observation
import cv2
import numpy as np
import rclpy
...
class ObserveViewer(Node):
    """Pair, display, and label the live Sys1 observation stream."""

    def __init__(self, args):
        super().__init__('sys1_observe_viewer')
        self.args = args
        ...
        self.create_subscription(
            Sys1Observation, args.observation_topic, self._on_observation, best)
        ...
        self.snapshot_observation_pub = self.create_publisher(
            Sys1Observation, snapshot + '/observation', reliable)
        self.state_pub = self.create_publisher(
            Sys1CalibrationState, args.state_topic, reliable)
    ...
    def _queue_click(self, source):
        ...
        if obs is None or not obs.state_ready or not obs.stand_locked:
            self.interlock = f'{source} ignored: sys0 is not in locked stand'
            return
    ...
    def _consume(self, slot):
        ...
        if not ready:
            if self.pending_click:
                self.pending_click = False
                self.interlock = 'CLICK CANCELLED: sys0 left locked stand'
            return
    ...
    def _draw(self):
        ...
        if obs is not None:
            ...
            lock = ('sys0: nominal lock OK' if obs.stand_locked else
                    'sys0: NOT IN LOCKED STAND')
        ...
        cv2.imshow('Sys1 observation calibration', np.hstack((rgb, depth, status)))
...
def main():
    ...
    parser.add_argument('--color-topic',
                        default='/vibe/sys1/observe/color/compressed')
    parser.add_argument('--depth-topic', default='/vibe/sys1/observe/depth')
    parser.add_argument('--labels-topic', default='/vibe/sys1/observe/labels')
    parser.add_argument('--camera-info-topic',
                        default='/vibe/sys1/observe/camera_info')
    parser.add_argument('--observation-topic',
                        default='/vibe/sys1/observe/observation')
    parser.add_argument('--click-topic',
                        default='/vibe/sys1/calibration/click')
    parser.add_argument('--snapshot-prefix',
                        default='/vibe/sys1/calibration')
    parser.add_argument('--state-topic', default='/vibe/sys1/calibration/state')
