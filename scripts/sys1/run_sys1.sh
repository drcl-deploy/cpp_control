...
usage() {
  cat <<'EOF'
Usage: run_sys1.sh [--color N]

Opens the sys1 console: the planner's state pane, and the one place the target
...
if [[ -z "${ros_setup}" ]]; then
  echo "run_sys1: no supported ROS setup found under /opt/ros" >&2
  exit 1
fi
if [[ ! -f "${workspace_setup}" ]]; then
  echo "run_sys1: workspace is not built: ${workspace_setup}" >&2
  exit 1
fi
...
echo "run_sys1: console on ROS domain ${ROS_DOMAIN_ID:-0}"
exec python3 "${script_dir}/sys1_console.py" "$@"
