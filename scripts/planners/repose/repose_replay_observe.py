#!/usr/bin/env python3
"""Read-only, frame-steppable replay viewer for The planner calibration bags."""
...
STATE_TOPIC = '/vibe/planner/calibration/state'
COLOR_TOPIC = '/vibe/planner/calibration/color/compressed'
DEPTH_TOPIC = '/vibe/planner/calibration/depth'
LABELS_TOPIC = '/vibe/planner/calibration/labels'
CAMERA_INFO_TOPIC = '/vibe/planner/calibration/camera_info'
OBSERVATION_TOPIC = '/vibe/planner/calibration/observation'
REQUIRED_TOPICS = (
    STATE_TOPIC,
    COLOR_TOPIC,
    DEPTH_TOPIC,
...
def _latest_bag(root):
    ...
    if not candidates:
        raise RuntimeError(f'no completed The planner observation bags under {root}')
...
class CalibrationBag:
    ...
    def _load(self):
        ...
        if missing:
            raise RuntimeError(
                'not a clicked The planner observation bag; missing topics: ' +
                ', '.join(missing))
        classes = {
...
class ReplayWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, bag, depth_min, depth_max, fps):
        ...
        self.fps = fps
        self.shortcuts = []
        self.setWindowTitle(f'The planner observation replay — {bag.path.name}')
...
def main():
    ...
    try:
        bag_path = (args.bag if args.bag else _latest_bag(
            os.environ.get('REPOSE_OBSERVE_BAG_DIR', './repose_observe_bags')))
        bag = CalibrationBag(bag_path)
    except RuntimeError as error:
        print(f'replay_observe: {error}', file=sys.stderr)
        return 1
