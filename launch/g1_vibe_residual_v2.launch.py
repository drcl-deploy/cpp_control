"""
Launch G1 VIBE Residual V2 — Extended HLC Observation.

Same as v1 but with enriched HLC obs (ang_vel, motion commands, anchor).

Usage:
    ros2 launch cpp_control g1_vibe_residual_v2.launch.py
    ros2 launch cpp_control g1_vibe_residual_v2.launch.py hlc_onnx_model_path:=/path/to/hlc.onnx
    ros2 launch cpp_control g1_vibe_residual_v2.launch.py wandb_path:=entity/project/run_id
"""
...
def _resolve_wandb_hlc(wandb_path: str) -> str:
    """Fetch `exported/policy.onnx` from the wandb run and return local path."""
    import wandb
...
def generate_launch_description():
    ...
    default_config = os.path.join(pkg, 'config', 'vibe', 'g1_residual_v2.yaml')
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument('wandb_path', default_value=wandb_path,
                              description='Optional wandb run path (entity/project/run_id). '
        ...
        Node(
            package='cpp_control',
            executable='g1_vibe_residual_v2_node',
            name='g1_vibe_residual_v2_node',
            output='screen',
            parameters=[{
