...
std::vector<float> as_float_vector(const YAML::Node& node,
                                   size_t broadcast_to) {
  std::vector<float> out;
  if (node.IsScalar())
    out.assign(broadcast_to, node.as<float>());
  else
    for (const auto& v : node) out.push_back(v.as<float>());
  return out;
}
...
int PortSpec::dim() const {
  int d = 1;
  for (int64_t s : shape) d *= static_cast<int>(s > 0 ? s : 1);
  return d;
}
...
DeployManifest DeployManifest::load(const std::string& path) {
  ...
  if (m.schema != "vibe.onnx.v1")
    throw std::runtime_error("deploy manifest: unsupported schema '" +
                             m.schema + "' in " + path);

  m.task_id = root["task_id"].as<std::string>("");
  m.run_path = root["run_path"].as<std::string>("");
  ...
  for (const auto& in : root["inputs"]) {
    ...
    for (const auto& s : in["shape"]) port.shape.push_back(s.as<int64_t>());
    for (const auto& g : in["groups"])
      port.groups.push_back(g.as<std::string>());
    for (const auto& t : in["terms"]) {
      TermSpec term;
      term.name = t["name"].as<std::string>();
      for (const auto& s : t["shape"]) term.shape.push_back(s.as<int64_t>());
      term.dim = t["dim"].as<int>();
      term.offset = t["offset"].as<int>();
      term.history = t["history_length"].as<int>(0);
      const auto source = t["source"];
      if (source) {
        term.source.kind = source["kind"].as<std::string>("");
        term.source.sensor = source["sensor"].as<std::string>("");
        term.source.model_name = source["model_name"].as<std::string>("");
        term.source.model_dtype = source["model_dtype"].as<std::string>("");
        term.source.slice = source["slice"].as<std::string>("");
      }
...
const PortSpec* DeployManifest::find_group(const std::string& group) const {
  for (const auto& p : inputs)
