...
std::vector<float> load_contact(const std::string& motion_path, int num_frames)
{
    ...
    if (!npz.count("matrix") || !npz.count("body_names"))
        throw std::runtime_error("Motion: contact_matrix.npz needs 'matrix' and "
                                 "'body_names': " + path);
    const auto& mat = npz.at("matrix");
    const auto names = to_strings(npz.at("body_names"));
    if (mat.shape.size() != 3 || mat.word_size != 1)
        throw std::runtime_error("Motion: contact matrix must be (T, N, N) int8: " + path);
    if (static_cast<int>(mat.shape[0]) != num_frames)
        throw std::runtime_error("Motion: contact matrix has " +
                                 std::to_string(mat.shape[0]) + " frames, motion has " +
                                 std::to_string(num_frames) + ": " + path);
    const size_t n = mat.shape[1];
    if (mat.shape[2] != n || names.size() != n)
        throw std::runtime_error("Motion: contact legend/matrix size mismatch: " + path);
    ...
    const size_t obj = n - 1;  // object is ALWAYS the last column (ContactSchedule)
...
Motion Motion::from_wire(int num_frames, const float* rows, int cols)
{
    ...
    if (cols != WIRE_COLS_MIN && cols != WIRE_COLS_FULL)
        throw std::runtime_error("Motion: wire cols " + std::to_string(cols) + " is neither " +
                                 std::to_string(WIRE_COLS_MIN) + " nor " +
                                 std::to_string(WIRE_COLS_FULL));
    ...
    m.bodywise_contact.assign(static_cast<size_t>(num_frames) * NUM_CONTACT_BODIES, 0.0f);
    ...
    for (int t = 0; t < num_frames; ++t)
    {
        ...
        std::memcpy(&m.bodywise_contact[static_cast<size_t>(t) * NUM_CONTACT_BODIES], tail + 6,
                    NUM_CONTACT_BODIES * sizeof(float));
    }
    return m;
}
