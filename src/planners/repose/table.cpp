...
std::vector<char> zip_member(const std::string& path, const std::string& name) {
  ...
  if (!in) throw std::runtime_error("the planner table: cannot open " + path);
  ...
  while (in.read(header.data(), 30)) {
    ...
    if (inflateInit2(&zs, -MAX_WBITS) != Z_OK)
      throw std::runtime_error("the planner table: inflate init failed for " + name);
    const int rc = inflate(&zs, Z_FINISH);
    inflateEnd(&zs);
    if (rc != Z_STREAM_END)
      throw std::runtime_error("the planner table: corrupt member '" + name + "'");
    return out;
  }
  return {};
}
...
Npy npy_parse(std::vector<char> blob, const std::string& what) {
  if (blob.size() < 12 || std::memcmp(blob.data(), "\x93NUMPY", 6) != 0)
    throw std::runtime_error("the planner table: '" + what + "' is not a .npy member");
  ...
  if (descr.size() < 2)
    throw std::runtime_error("the planner table: '" + what + "' has no dtype");
...
Npy need(const std::string& path, const char* key) {
  ...
  if (blob.empty())
    throw std::runtime_error("the planner table: '" + std::string(key) +
                             "' missing in " + path);
...
std::vector<float> floats(const Npy& a) {
  ...
  if (a.kind == 'f' && a.word == 4)
    ...
    throw std::runtime_error("the planner table: expected a float array");
  return out;
}
...
std::vector<int> ints(const Npy& a) {
  ...
  if ((a.kind == 'i' || a.kind == 'u') && a.word == 4)
    ...
    throw std::runtime_error("the planner table: expected an integer array");
  return out;
}

std::vector<uint8_t> bools(const Npy& a) {
  if (a.kind != 'b')
    throw std::runtime_error("the planner table: expected a bool array");
...
ClipTable ClipTable::load(const std::string& path) {
  ...
  if (clean.size() != n)
    throw std::runtime_error("the planner table: 'clean' must be (N,) bool: " + path);
...
void ClipTable::load_frames_retargeted(const std::string& library_npz,
  ...
  if (motion_files_.size() != clip_start_.size())
    throw std::runtime_error(
        "the planner table: " + library_npz + " lists " +
  ...
  for (size_t c = 0; c < motion_files_.size(); ++c) {
    ...
    if (m.num_frames != clip_len_[c])
      throw std::runtime_error("the planner table: " + path + " has " +
    ...
    for (int i : mine) {
      ...
      if (lo < 0 || hi >= m.num_frames || hi < lo)
        throw std::runtime_error("the planner table: row " + std::to_string(i) +
...
void ClipTable::load_frames_baked(const std::string& path) {
  ...
  if (off.size() != rows_.size() || len.size() != rows_.size())
    throw std::runtime_error("the planner bundle: row count != table row count: " +
                             path);
  if (rows.count % static_cast<size_t>(cols_) != 0)
    throw std::runtime_error("the planner bundle: rows are not a multiple of " +
                             std::to_string(cols_) + " wide: " + path);
  frames_ = floats(rows);
  stand_ = floats(need(path, "stand"));
  if (static_cast<int>(stand_.size()) != cols_)
    throw std::runtime_error("the planner bundle: stand row is the wrong width");
  for (size_t i = 0; i < rows_.size(); ++i) {
...
void ClipTable::set_nominal_stand(const std::vector<float>& default_angles_mj) {
  if (cols_ == 0 || stand_.size() != static_cast<size_t>(cols_))
    throw std::runtime_error(
        "the planner table: load frames before setting the nominal stand");
  if (default_angles_mj.size() != static_cast<size_t>(g1::NUM_JOINTS))
    throw std::runtime_error("the planner table: nominal stance is not " +
...
void ClipTable::bake(const std::string& out) const {
  if (frames_.empty()) throw std::runtime_error("the planner bake: no frames loaded");
