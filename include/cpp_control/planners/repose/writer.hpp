...
/// Plan -> the reference rows the controller plays. The whole seam, and it is small:
...
/// collapses to one scalar (docs/planners/repose/planner.md §1). Rows come straight
...
/// Live robot state the seam blends onto — everything it reads from the controller.
struct LiveState {
  std::vector<float> joint_pos_il;  ///< (29,) IL-ordered, radians
  std::vector<float> joint_vel_il;
  /// v7.1 only, and empty otherwise: the last row of the reference the controller is
  /// CURRENTLY playing. A ramp that starts from what the controller was actually told is
...
class ReferenceWriter {
 public:
  ...
  /// reckoning from the pose it started at (docs/planners/repose/planner.md §7.1).
  ...
  /// The last row of the reference just built — what the controller will be holding when
